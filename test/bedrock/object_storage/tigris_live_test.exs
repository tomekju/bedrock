defmodule Bedrock.ObjectStorage.TigrisLiveTest do
  ...
  @live_tigris_ready? Enum.all?(
                      ...
                      ) and System.get_env("BEDROCK_LIVE_TIGRIS") == "1"

  if !@live_tigris_ready? do
    @moduletag skip: "requires BEDROCK_LIVE_TIGRIS=1 and configured BEDROCK_S3_* credentials"
  end
  ...
  defp endpoint_config do
    ...
    case URI.parse(endpoint) do
      %URI{scheme: "https", host: host, port: port, path: path, query: nil, fragment: nil}
      when is_binary(host) and host != "" and path in [nil, "", "/"] ->
        [scheme: "https://", host: host, port: port || 443]

      _ ->
        raise "invalid required Tigris endpoint configuration"
    end
  end
  ...
  defp assert_prefix_empty(backend, prefix) do
    assert [] = backend |> ObjectStorage.list(prefix) |> Enum.to_list()
  end
